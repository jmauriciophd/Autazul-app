 ...
 * 
 * INSTRUÇÕES:
 * 1. Salve a logo como: /assets/logo.png
 * 2. Salve o background como: /assets/bg.png
 */

// Logo principal - coração puzzle autismo
// Para funcionar: salvar imagem como /assets/logo.png
let logoUrl: string;
try {
  logoUrl = new URL('./logo.png', import.meta.url).href;
} catch {
  // Fallback SVG se logo.png não existir
...
// Para funcionar: salvar imagem como /assets/bg.png
let bgUrl: string;
try {
  bgUrl = new URL('./bg.png', import.meta.url).href;
} catch {
  // Fallback gradiente SVG se bg.png não existir
  bgUrl = 'data:image/svg+xml;base64,' + btoa(`
...
export const loginBackground = bgUrl;
